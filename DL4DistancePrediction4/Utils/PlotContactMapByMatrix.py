...
if not os.path.isfile(contactFile):
	print 'the contact file (in matrix format) does not exist: ', contactFile
	exit(-1)

contactMatrix = np.loadtxt(contactFile)
lenseq = contactMatrix.shape[0]

## set the diagnoal lines to zeros
for i in xrange(6):
	np.fill_diagonal(contactMatrix[i:], 0)
	np.fill_diagonal(contactMatrix[:,i:], 0)

## convert probability values to RGB, note that colors is similar to just a 1D array.
colors = []
for y in contactMatrix.flatten():
