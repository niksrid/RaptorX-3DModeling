...
def Usage():
	...
	print '-i: a folder containing the input features of all proteins in the list file, each file ending with .propertyFeatures.pkl'
    	print '-d: a folder for result save (default current work directory). The result file is named after proteinName.predictedProperties.pkl'
    	...
    	print '-g: the ground truth folder containing native property in PKL format'
...
def main(argv):
	...
	if len(argv) < 6:
    	...
    	try:
       		opts, args = getopt.getopt(argv, "p:i:m:g:d:", ["proteinNameFile=", "inputFolder=", "model=", "nativefolder=", "savefolder="])
        	#print opts, args
	...
	if not os.path.isfile(proteinNameFile):
		print 'ERROR: protein name file does not exist: ', proteinNameFile
		exit(1)

	if not os.path.isdir(inputFolder):
		print 'ERROR: input feature folder does not exist: ', inputFolder
		exit(1)
	...
	batchSize = 500
	for i in range(0, len(proteins), batchSize):
		subset = proteins[i: min(i+batchSize, len(proteins)) ]
		predFiles = [ os.path.join(inputFolder, protein + '.propertyFeatures.pkl') for protein in subset ]
